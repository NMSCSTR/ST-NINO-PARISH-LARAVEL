                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                <tr class="hover:bg-gray-50 transition">
                                    ...
                                    <td class="px-4 py-2 text-right">
                                        ...
                                        <div id="modal-{{ $reservation->id }}" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
                                            <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6 relative">
                                                <h3 class="text-lg font-semibold text-gray-800 mb-4">Approve / Reject Reservation</h3>

                                                <form action="{{ route('priest.reservations.approve', $reservation->id) }}" method="POST" class="mb-3">
                                                    @csrf
                                                    <textarea name="remarks" placeholder="Add remarks (optional)" class="w-full border px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 mb-2" rows="3"></textarea>
                                                    <button type="submit" class="w-full flex items-center justify-center gap-2 text-green-600 font-medium hover:bg-green-50 rounded px-3 py-2">
                                                        Approve
                                                    </button>
                                                </form>
                                                ...
                                                <form action="{{ route('priest.reservations.reject', $reservation->id) }}" method="POST">
                                                    @csrf
                                                    <textarea name="remarks" placeholder="Add remarks (optional)" class="w-full border px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 mb-2" rows="3"></textarea>
                                                    <button type="submit" class="w-full flex items-center justify-center gap-2 text-red-600 font-medium hover:bg-red-50 rounded px-3 py-2">
                                                        Reject
                                                    </button>
                                                </form>

                                                <button type="button" onclick="closeModal({{ $reservation->id }})" class="absolute top-3 right-3 text-gray-400 hover:text-gray-600">
                                                    ✕
                                                </button>
                                            </div>
                                        </div>
                                        ...
                                        <!-- Details Modal -->
                                        <div id="details-modal-{{ $reservation->id }}" class="fixed inset-0 bg-black/60 hidden items-center justify-center z-50">
                                            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-3xl p-6 relative overflow-y-auto max-h-[90vh]">
                                                <div class="flex items-center justify-between mb-6">
                                                    ...
                                                    <button onclick="closeDetailsModal({{ $reservation->id }})" class="text-gray-400 hover:text-gray-600 text-xl">✕</button>
                                                </div>
                                                <!-- Member Info -->
