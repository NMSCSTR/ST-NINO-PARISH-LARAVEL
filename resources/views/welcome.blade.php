...
<section class="relative py-20 bg-gradient-to-br from-[#f9f6f0] to-[#f2e8dc] text-[#2c1c0f] overflow-hidden" id="events">
...
<!-- ⬆️ Scroll Up Button (appears after scroll) -->
...
<!-- ⬇️ Scroll Down Button (always visible before scrolling) -->
<a href="#events" id="scrollDownBtn"
    class="fixed bottom-8 left-8 p-3 bg-gradient-to-r from-[#d4af37] to-[#8b5e3c] text-white rounded-full shadow-lg flex items-center justify-center hover:scale-110 hover:shadow-xl transition-all duration-300 z-50">
    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 animate-bounce" fill="none" viewBox="0 0 24 24"
        stroke="currentColor" stroke-width="2">
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Show scroll up when user scrolls down
    window.addEventListener("scroll", () => {
        if (window.scrollY > 300) {
            scrollUpBtn.classList.remove("hidden");
            scrollDownBtn.classList.add("hidden"); // hide down button once scrolled
        } else {
            scrollUpBtn.classList.add("hidden");
            scrollDownBtn.classList.remove("hidden"); // show down button at top
        }
    });
    ...
    // Scroll down smoothly to #events
